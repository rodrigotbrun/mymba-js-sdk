...
function splitUrl(partsStr, {
  protocolRelative
}) {
  const match = protocolRelative && partsStr.match(protocolRelativeUrlRegExp) || partsStr.match(defaultUrlRegExp) || [];
  const beforePathname = match[1] || '';
  const pathname = (match[2] || ''). // Remove leading slashes
  replace(/^\/+/, '') // Remove trailing slashes
  .replace(/\/+$/, '') // Normalize consecutive slashes to just one
  .replace(/\/+/g, '/');
...
function urlJoin(...parts) {
  ...
  const partsStr = parts.filter(part => typeof part === 'string' || typeof part === 'number').join('/'); // Split the parts into beforePathname, pathname, and afterPathname
  ...
  const {
    ...
    pathname,
    afterPathname
  } = splitUrl(partsStr, options);
  let url = ''; // Start with beforePathname if not empty (http://google.com)

  if (beforePathname) {
    url += beforePathname + (pathname ? '/' : ''); // Otherwise start with the leading slash
  } else if (options.leadingSlash) {
    url += '/';
  } // Add pathname (foo/bar)


  url += pathname; // Add trailing slash
  ...
  const query = { ..._queryString.default.parse(afterPathname, options.queryOptions),
    ...options.query
  };
